...
{
    /**
     * Constructs a new WorkerAccount instance
     * @constructor
    ...
    constructor(config)
    {
        ...
        // Register custom aggregate operators
        ...
        this.prev_mailboxes_results = new LRUCache(10);
        this.prev_conversation_results = new LRUCache(10);
        this.sync_timeout = null;
        this.in_sync = false;
    }

    /**
     * Register a callback notification (poor-mans EventEmitter)
    ...
    /**
     * Remove a previously registered callback notification
     * @param {string} eventName Name of the event
     * @param {function} callback Callback function to remove (ignored)
     */
    ...
    /**
     * Opens the account and initiates a synchronize
     * @async
     * @returns {Promise<void>} A promise for the new drev number
     */
    openAndSync()
    {
    ...
    /**
     * Performs a synchronize between the IMAP server and the DB
     * @async
     * @returns {Promise<number>} The promise for the new drev number
     */
    sync()
    {
    ...
    async #open_nolock()
    {
        ...
        if (this._imap == null)
        {
            ...
            this._imap.on('mail', () => this.#on_imap_change_detected());
            this._imap.on('expunge', () => this.#on_imap_change_detected());
            this._imap.on('uidvalidity', () => this.#on_imap_change_detected());
            this._imap.on('update', () => this.#on_imap_change_detected());
        }
    ...
    /**
     * Handler for when an IMAP change is detected (typically on the INBOX).
     * Schedules a resync soonish (unless already in sync)
     */
    #on_imap_change_detected()
    {
        // This check is necessary as switching between mailboxes during
        // the sync can trigger this method
        if (this.in_sync)
            return;

        // Schedule a new sync really soon...
        this.#schedule_next_sync(100);
    }

    /**
     * Schedules the next sync operations
     * @param {Number} timeout milliseconds before next sync
     */
    #schedule_next_sync(timeout)
    {
        if (this.config.no_auto_sync)
            return;
    ...
    /**
     * Report sync progress updates
     * @param {Number} complete Percentage complete (0..100 or -1 for error) 
     * @param {String} message A message describing the current progress step
     */
    ...
    async #sync_nolock()
    {
        ...
        // Don't sync if less than 5 seconds since the last one finished
        if (!this.config.no_auto_sync)
        {
            let throttleSync = this.lastSyncTime + 5000 - startTime;
            if (throttleSync > 0)
            {
                // Instead schedule a new sync when the 5 seconds is up
                this.#schedule_next_sync(throttleSync);
                return;
            }
        }

        // Start sync
        this.info && this.info(`sync starting for ${this.config.host} ${this.config.user}`);
        this.in_sync = true;
        try
        {
            // Retry up to 3 times if imap error detected
            let retryCount = 0;
            while (true)
            {
                try
        ...
        {
            this.in_sync = false;
            this.info && this.info(`...sync finished in ${Date.now() - startTime} ms.`);
        }
    }
    ...
    async #sync_internal()
    {
        ...
        try
        {
            // Make sure IMAP session is connected
            if (!this._imap)
            {
                this._imap = new Imap(this.config);
                await this._imap.connect();
            ...
            // Get list of mailboxes from DB
            let db_boxes = this.db.all(SQL.select("data").from("mailboxes")).map(x => JSON.parse(x.data));
            
            // Get list of mailboxes from IMAP
            ...
            // Get list of mailboxes and re-order so the INBOX
            // is last (so it's left open for monitoring)
            let arrMailboxes = [...mailboxes.values()];
            let inboxIndex = arrMailboxes.findIndex(x => x.name == "INBOX");
            if (inboxIndex >= 0)
            {
                arrMailboxes.push(arrMailboxes[inboxIndex]);
                arrMailboxes.splice(inboxIndex, 1);
            }
            ...
            let modified = false;
            this.db.transactionSync(() => {
                modified = this.#build_conversations();
            })
            ...
            // Notify
            this.sync_callback && this.syncCallback({
                drev: this.drev,
                time: this.lastSyncTime,
            });

            // Schedule next sync
            this.#schedule_next_sync(60000);
        ...
        {
            console.error(`Synchronize Failed - ${err.message}`);
            this.#progress(-1, `Synchronize Failed - ${err.message}`);
            throw err;
        }
    }

    /**
     * Helper to rebuild conversations after DB sync with IMAP
     * @returns true if DB was modified, otherwise false
     */
    #build_conversations()
    {
        // Prepare
        ...
        */
        ...
        let newConversationCount = this.#get_conversations(rebuild_mids);
    ...
    /**
     * Helper to build the conversations for a set of message-ids
     * @param {Set<string>} mids A set of message-ids
     * @returns {Number} the number of created conversations
     */
    #get_conversations(mids)
    {
        if (mids.size == 0)
            return 0;
    ...
    /**
     * Get a mailbox by case-insensitive name
     * @param {string} name Name of the mailbox to find
     * @returns The mailbox reference
     */
    ...
    /**
     * Implements the /api/folders endpoint
     * @param {Object} options 
     * @returns {Object} An object ready for JSON serialization
     */
    ...
    /**
     * Implements the /api/conversations API endpoint
     * @param {Object} options 
     * @returns {Object} An object ready for JSON serialization
     */
    ...
    /**
     * Implements the /api/conversations_and_mailboxes API endpoint
     * @param {Object} options 
     * @returns {Object} An object ready for JSON serialization
     */
    ...
    /**
     * Implements the /api/conversation API endpoint
     * @param {Object} options 
     * @returns {Object} An object ready for JSON serialization
     */
