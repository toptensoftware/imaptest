...
class WorkerAccount
{
    ...
    async open_nolock()
    {
        ...
        if (this._imap == null)
        {
            ...
            this._imap.on('mail', () => this.resync_soon());
            this._imap.on('expunge', () => this.resync_soon());
            this._imap.on('uidvalidity', () => this.resync_soon());
            this._imap.on('update', () => this.resync_soon());
        }
    ...
    async sync_nolock()
    {
        ...
        if (throttleSync > 0)
        {
            ...
            this.scheduleNextSync(throttleSync);
            return;
        }

        this.info && this.info("sync starting...");
        this.in_sync = true;
        try
        {
            let retryCount = 0;
    ...
    async #sync_internal()
    {
        ...
        try
        {
            ...
            // Move the inbox to the end so it's left open for monitoring
