...
class WorkerAccount
{
    ...
    constructor(config)
    {
        ...
        this.info = console.log;//config.info;
        ...
        this.sync_revision = Date.now();
    }


    // Returns a revision number that can be used to 
    ...
    async #sync_internal()
    {
        ...
        // Remember and return sync revision
        if(modified)
        {
            this.sync_revision = Date.now();
        }
        return this.sync_revision;
    }
    ...
    get_mailboxes_nolock()
    {
        if (this.cache_mailboxes?.revision == this.sync_revision)
        {
        ...
        return this.cache_mailboxes = {
            mailboxes: [...boxes.values()],
            revision: this.sync_revision,
        }
    }
    ...
    get_mailboxes()
    {
        return this.lock.section(() => {
            return this.get_mailboxes_nolock();
        });
    }
    ...
    get_conversations_and_mailboxes(options)
    {
        return this.lock.section(() => {
            ...
            let mailboxes = this.get_mailboxes_nolock();
