...
class Account
{
    ...
    getConversations(mids)
    {
        ...
        let s_insert_conv = this.db.prepareCached(new SQL()
            ...
            .values('conversation_id, date, subject, flags'.split(','))
        );
        ...
        for (var g of gb.groups)
        {
            ...
            let conv_rid = s_insert_conv.run(messages[0].message_id, lastMessage.date, lastMessage.subject, g.flags).lastInsertRowid;
