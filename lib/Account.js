...
const { message_flags_mask } = require('./Utils');
...
class Account
{
    ...
    getConversations(mids)
    {
        ...
        let s_insert_conv = this.db.prepareCached(new SQL()
            ...
            .values('conversation_id, date, subject, flags, participants'.split(','))
        );
    ...
    get_conversations(options)
    {
        let queryFetch;
        let queryCount;
        let fields = "conversations.date as date, conversation_id, flags, participants, subject";
        if (options.mailbox)
        {
            queryFetch = new SQL()
                .select(fields)
