...
class Account
{
    ...
    async status()
    {
        ...
        return {
            ...
            modifiedMessages: await messages_collection.countDocuments({ state: { $gt: 0} } ),
    ...
    async dropAllConversations()
    {
        await Database.transaction(async (tx) => {
            ...
            // Delete all flagged conversations
            r = await messages_collection.deleteMany({ state: { $lt: 0 } } );
            console.log(`Deleted ${r.deletedCount} messages`);

            // Mark all messages as modified
            r = await messages_collection.updateMany({}, {$set: { state: 1 } } );
            console.log(`Marked ${r.modifiedCount} messages modified`);
        });
