...
class TransactionCollection
{
    ...
    {
        return this._coll.updateMany(filter, update, this.mergeSession(options));
    }
...
class Database
{
    async open(config)
    {
        this._client = await MongoClient.connect(config.db_server);
        this._db = this._client.db(config.db_name);
    }
