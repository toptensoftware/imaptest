...
class Mailbox
{
    ...
    async sync(tx, sync_notify)
    {
        ...
        let sync_info = 
        {
            ...
            messages_collection: tx.collection(this._user.collection_name("messages")),
            sync_notify: sync_notify
        }
        
        
        console.log(`syncing ${this._user.collection_name("messages")} ${this.name} ${sync_info.box.messages.total} messages...`)
    ...
    async updateDb(tx)
    {
        await tx.collection(this._user.collection_name("mailboxes")).updateOne(
            {
                name: this._name,
            }, 
            {
     ...
     #fetch(sync_info, range, is_uid_range)
     {
         ...
         return kind.fetchHeaders(range, 
            ...
            (msg) => 
            {
                ...
                sync_info.sync_notify.message_added?.(m)
            }
        );
     }
