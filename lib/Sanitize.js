...
class Sanitize
{
    ...
    static options = {
        ...
        allowedAttributes: {
            "*": [ 'class', 'style', 'align', 'valign', 'nowrap', 'border', 'dir', 'width', 'height', 
                    'aria-hidden', 'rel', 'role', 'color', 'bgcolor', 'title' , 'quote_reply_hack'],
            a: [ 'href', 'name', 'target' ],
        ...
        transformTags: {
            ...
            'div': function (tagName, attribs)
            {
                ...
                let isQuote = classes.findIndex(x => x.endsWith("gmail_quote") || x == "quoted_reply")
                if (isQuote >= 0)
                {
                    tagName = "details";
                    attribs.title = "Show quoted reply";
                    attribs.quote_reply_hack = "***";           // This will be stripped out via regex later
                    if (classes.findIndex(x => x == "quoted_reply") < 0)
                    {
                        classes.push("quoted_reply");
    ...
    static Html(html, cidMapper)
    {
        ...
        // Insert our "..." symbol for the quoted reply details disclosure
        clean = clean.replace(/ quote_reply_hack="\*\*\*">/g, "><summary><span>···</span></summary>")
