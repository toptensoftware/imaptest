...
/**
 * Manages synchronization of an IMAP account with the DB
 */
class WorkerMailbox
{
    /**
     * Constructs a new WorkerMailbox instance
     * @constructor
     * @param {Account} account A reference to the owning WorkerAccount object
    ...
    /**
     * Initializes this WorkerMailbox instance from data previously saved to the DB
     * @param {object} data The previously saved mailbox data
     */
    initFromDb(data)
    {
        this.data = data;
    ...
    /**
     * Initializes this WorkerMailbox instance from a newly discovered IMAP mailbox
     * @param {object} mailbox The mailbox data as returned from IMAP
     */
    ...
    /**
     * Fetch a range of messages from the IMAP server and add them to the database
     * 
     * @async
     * @param {object} sync_info Context info for the current sync operation
     ...
     * current UIDs and removing any from the db that no longer exist
     * @async
     * @param {object} sync_info Context info for the current sync operation
     * @returns {Promise<void>}
     */
    async #trimDeletedMessages(sync_info)
    {
        ...
        {
            // Prepare statement
            let s_update = sync_info.db.prepareCached("UPDATE messages SET state=-1 WHERE MAILBOX=? AND uid=? AND state <> -1 ");
            for (let u of uidsToDelete)
            {
                s_update.run(this.name, u);
            }
