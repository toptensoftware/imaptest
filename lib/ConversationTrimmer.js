...
class ConversationTrimmer
{
    ...
    static async trimConversations(account)
    {
        ...
        if (conversationCount != 0)
        {
            ...
            info && info(` - total affecting messages: ${affected_mids.length}`);
            if (affected_mids.length > 0)
            {
                ...
                await Utils.batch_work(affected_mids, 1000, async (batch) => {
                    ...
                    await ccoll.find(
                        ...
                        { projection: { _id: 1 } }                
                    ).forEach(x => affected_conversations.push(x._id));

                });

                info && info(` - deleting ${affected_conversations.length} affected conversations`);
        ...
        info && info(` - finished in ${Date.now() - start} ms`);
    }
}
