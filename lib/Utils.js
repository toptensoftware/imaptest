...
class Utils
{
    ...
    static encrypt(text)
    {
        ...
        const cipher = crypto.createCipheriv("aes-256-ctr", "3Q02pJrfVbptLmab9kqWA73S027fsqBe", iv);
    ...
    static decrypt(content, iv)
    {
        const decipher = crypto.createDecipheriv("aes-256-ctr", "3Q02pJrfVbptLmab9kqWA73S027fsqBe", Buffer.from(iv, 'hex'));
    ...
    {
        return Utils.encrypt(JSON.stringify(obj));
    };
    
    static decryptJson(content, iv)
    {
        return JSON.parse(Utils.decrypt(content, iv));
    };
}
