...
class Utils
{
    ...
    static get_email_address_from_headers(hdrs, kind)
    {
        return Utils.get_combined_header_value(hdrs, kind);
    }

    static get_email_addresses_from_headers(hdrs)
    {
        let r  = {
            from: Utils.get_email_address_from_headers(hdrs, 'from'),
            to: Utils.get_email_address_from_headers(hdrs, 'to'),
            cc: Utils.get_email_address_from_headers(hdrs, 'cc'),
            bcc: Utils.get_email_address_from_headers(hdrs, 'bcc'),
        }

        // Remove falsey values
        return _.pickBy(r, _.identity);
    }

    static participants_from_headers(hdrs)
    {
        let list = _.compact([
            Utils.get_combined_header_value(hdrs, 'from'),
            Utils.get_combined_header_value(hdrs, 'to'),
            Utils.get_combined_header_value(hdrs, 'cc'),
            Utils.get_combined_header_value(hdrs, 'bcc'),
        ]);
