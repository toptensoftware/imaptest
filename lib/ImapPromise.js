...
class ImapPromiseBase
{
    ...
    static rawBodyHandler(item)
    {
        ...
        return function(stream)
        {
            ...
            stream.on('end', () => {
                item.body = Buffer.concat(chunks);
            });
        }
    }

    static textBodyHandler(item, encoding)
    {
        let chunks = [];
        return function(stream)
        {
            ...
            stream.on('end', () => {
                item.body = Buffer.concat(chunks).toString(encoding ?? 'binary');
                item.headers = Imap.parseHeader(item.body);
            });
        }
    }

    fetchAll(uids, options)
    {
        return new Promise((resolve, reject) => {
            ...
            let bodyHandler = options.bodyHandler ?? ImapPromise.rawBodyHandler;
    ...
    */
    ...
    async fetchHeaders(uids, options, callback)
    {
        let result = callback ? undefined : [];
        await this.fetch(uids, options, (msg, seqno) => {
    ...
    fetchHeadersOld(uids, options, callback)
    {
        return new Promise((resolve, reject) => {
...
ImapPromise.parseHeader = Imap.parseHeader;
